...
class HadoopJobRunner(MRJobRunner):
    ...
    def _run_job_in_hadoop(self):
        self._counters = []
        self._step_ids = [None] * len(self._get_steps())
        ...
        for step_num in xrange(self._num_steps()):
            ...
            try:
                ...
                if pid == 0:  # we are the child process
                    ...
                    self._process_stderr_from_streaming(master, step_num)
                    _, returncode = os.waitpid(pid, 0)
                    master.close()

            if returncode == 0:
                # parsing needs step number for whole job
                self._fetch_counters(step_num)#[step_num + self._start_step_num])
    ...
    def _fetch_counters(self, step_num, skip_s3_wait=False):
        ...
        self._counters.appends(new_counters.get(hadoop_step_num,{}))
